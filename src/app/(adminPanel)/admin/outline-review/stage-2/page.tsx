...
export default function Stage2ApprovalPage() {
  const [loading, error, data, refetch] = useFetchData(approvalService.getStage2);
  ...
  if (!hasPermission(ENDPOINT_PERMISSIONS.approval.STAGE2)) {
  ...
  const handleApprove = async () => {
    ...
    try {
      setSubmitting(true);
      await approvalService.postStage2Approve(selected.outlineId);
      toast.success("Final approved");
      await refetch();
      setSelected(null);
    } catch (err) {
      console.error(err);
      toast.error("Failed to final approve. Refreshing list.");
      await refetch();
    } finally {
      setSubmitting(false);
    }
  ...
  const handleRequestChanges = async (body?: any) => {
    ...
    try {
      setSubmitting(true);
      await approvalService.postStage2RequestChanges(selected.outlineId, body);
  ...
  return (
    <div className="p-6 text-white space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Stage 2 Approval</h1>
        <p className="text-sm text-muted-foreground">Outlines awaiting final approval</p>
      </div>

      <Card className="bg-[#141414] border-white/10">
        <CardHeader>
          <CardTitle>Submissions</CardTitle>
          <CardDescription>List of outlines for stage 2 approval</CardDescription>
        </CardHeader>
        <CardContent>
          {outlines.length === 0 ? (
              ...
              <TableBody>
                {outlines.map((o: any) => (
                  <TableRow key={o.outlineId} onClick={() => openDetail(o.outlineId)} className="cursor-pointer">
                    ...
                    <TableCell>{o.lastReviewAt ? new Date(o.lastReviewAt).toLocaleString() : "—"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
                ...
                <div className="mt-3">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        setSubmitting(true);
                        await approvalService.postStage2Reject(selected.outlineId, { comment: "Rejected" });
